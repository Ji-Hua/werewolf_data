...
class BlackBatEngine(BaseParserEngine):
    def __init__(self):
        super().__init__()
        self.checkable_werewolf_roles = ["狼人", "黑蝙蝠"]
        self.werewolf_group_roles.append("黑蝙蝠")
        self.werewolf_camp_roles.append("黑蝙蝠")
    ...
    def _check_death(self, ability, target, round):
        ...
        if ability in ["毒杀", "守护"] and self.protection:
            ...
            bat_round_actions = self.clean_data[bat_seat]["actions"][round]
            bat_target = None
            for d in bat_round_actions:
                if d["ability"] == "庇护":
                    bat_target = d["target_seat"]
            if bat_target:
